          ...
          <div className="hidden lg:flex items-center gap-8">
            {navLinks.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className="text-sm font-medium text-gray-600 hover:text-brand-dark transition-colors relative after:absolute after:bottom-[-4px] after:left-0 after:w-0 after:h-0.5 after:bg-brand-green after:transition-all hover:after:w-full"
              >
                {link.label}
              </a>
            ))}
          ...
          <div className="px-6 py-4 space-y-1">
            {navLinks.map((link) => (
              <a
                ...
                className="block py-3 px-4 rounded-lg text-sm font-medium text-gray-600 hover:text-brand-dark hover:bg-gray-50 transition-colors"
              >
                {link.label}
              </a>
            ))}
